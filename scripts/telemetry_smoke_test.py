...
def simulation_payload() -> dict:
    ...
    lidar_points = [{"x": float(idx) * 0.02, "y": float(idx) * -0.01, "intensity": 0.5} for idx in range(MAX_LIDAR_POINTS + 12)]
    return {
        "drone_id": drone_id,
        ...
        "tdoa": {
            ...
            "visible_anchor_count": 4,
            "anchors": [
                {"id": "A0", "x": 0.0, "y": 0.0, "z": 2.5, "visible": True, "last_seen_ms": 9.0},
                {"id": "A1", "x": 8.0, "y": 0.0, "z": 2.5, "visible": True, "last_seen_ms": 10.0},
                {"id": "A2", "x": 0.0, "y": 8.0, "z": 2.5, "visible": True, "last_seen_ms": 11.0},
                {"id": "A3", "x": 8.0, "y": 8.0, "z": 2.5, "visible": True, "last_seen_ms": 12.0},
            ],
...
def main() -> int:
    ...
    target = next((drone for drone in drones if drone.get("drone_id") == payload["drone_id"]), None)
    if target is None:
        return fail(f"posted simulation telemetry for drone_id={payload['drone_id']} did not appear in fleet snapshot")
    if target.get("source") != "simulation":
        return fail(f"expected top-level source=simulation, got {target.get('source')!r}")
    ...
    if target.get("localization_data_source") != "simulation":
        return fail(f"expected localization_data_source=simulation, got {target.get('localization_data_source')!r}")
    ...
    if len(lidar_points) != MAX_LIDAR_POINTS:
        return fail(f"expected LiDAR points to be capped at {MAX_LIDAR_POINTS}, got {len(lidar_points)}")
    if target.get("lidar", {}).get("point_count") != MAX_LIDAR_POINTS:
        return fail(f"expected sanitized LiDAR point_count={MAX_LIDAR_POINTS}, got {target.get('lidar', {}).get('point_count')}")
    ...
    print(f"backend_mode={fleet.get('backend_mode')} simulation_enabled={fleet.get('simulation_enabled')} real_drone_count={fleet.get('real_drone_count')}")
    print(f"drone_id={payload['drone_id']} source={target.get('source')} lidar_points={len(lidar_points)}")
    return 0
