...
Every failing TSan report in `docs/performance-engineering-stability-validation/tsan_ctest_phase65.log` has the same warning class:
...
`docs/architecture-safety-validation-hardening/RACE_DETECTION_REPORT.md` records that the earlier TSan obstacle was narrowed to external `libOpenNI2.so.0` mutex misuse and that the race-clean run required handling that external issue separately from repository code.
