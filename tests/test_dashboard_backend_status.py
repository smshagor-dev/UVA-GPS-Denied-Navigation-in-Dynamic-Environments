...
class BackendStatusTests(unittest.TestCase):
    ...
    def test_operator_status_shows_simulation_warning(self) -> None:
        self.assertIn("LOCALIZATION:SIMULATION", compose_operator_status("simulation", "simulation"))

    def test_operator_status_warns_when_no_real_drones_connected(self) -> None:
        self.assertIn(
            "NO REAL DRONES",
            compose_operator_status("production", "real", simulation_enabled=False, real_drone_count=0),
        )
    ...
    def test_mission_start_allowed_rejects_simulation_or_low_confidence(self) -> None:
        self.assertFalse(mission_start_allowed(self._snapshot(localization_data_source="simulation")))
        self.assertFalse(mission_start_allowed(self._snapshot(localization_confidence=0.42)))
    ...
    def test_sensor_payload_parsers_preserve_real_and_simulation_sources(self) -> None:
        self.assertEqual(parse_camera_payload({"status": "live", "source": "real"})[1]["source"], "real")
        self.assertEqual(parse_lidar_payload({"status": "simulation", "source": "simulation"})[1]["source"], "simulation")
        self.assertEqual(parse_tdoa_payload({"status": "playback", "source": "playback"})[1]["source"], "playback")
