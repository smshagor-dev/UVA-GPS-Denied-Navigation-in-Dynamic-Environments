...
inline std::string_view to_string(DroneRole r) {
    switch (r) {
    case DroneRole::CANDIDATE:
        return "CANDIDATE";
    case DroneRole::FOLLOWER:
        return "FOLLOWER";
    case DroneRole::LEADER:
        return "LEADER";
...
public:
    ...
    explicit V2XMeshNetwork(uint32_t local_id, std::string multicast_group = "239.255.0.1",
                            uint16_t port = 7400);
    ~V2XMeshNetwork();
    ...
    [[nodiscard]] DroneRole local_role() const {
        return role_.load();
    }
    [[nodiscard]] uint32_t leader_id() const {
    ...
    void on_peer_update(PeerCallback cb) {
        peer_cb_ = std::move(cb);
    }

    //  Metrics
    [[nodiscard]] float avg_latency_ms() const {
        return avg_latency_ms_;
    }
    [[nodiscard]] uint32_t tx_count() const {
        return tx_count_;
    }
    [[nodiscard]] uint32_t rx_count() const {
        return rx_count_;
    }
    [[nodiscard]] float packet_loss_pct() const {
...
public:
    explicit LeaderFollowerController(uint32_t drone_id, std::shared_ptr<V2XMeshNetwork> net)
    ...
    [[nodiscard]] Eigen::Vector3d compute_target(const Eigen::Vector3d& leader_pos,
                                                 const FormationCommand& cmd,
                                                 uint32_t follower_index) const;

    // P-controller step  velocity command (m/s)
    [[nodiscard]] Eigen::Vector3d velocity_command(const Eigen::Vector3d& current_pos,
    ...
    velocity_command(const Eigen::Vector3d& current_pos, const Eigen::Vector3d& current_velocity,
                     const Eigen::Vector3d& target_pos, const std::vector<PeerInfo>& peers,
                     const std::vector<AvoidanceObstacle>& obstacles = {}, float kp = 1.5f,
                     float max_speed_mps = 4.0f) const;
    ...
    [[nodiscard]] Eigen::Vector3d compute_avoidance_velocity(
        ...
        float min_separation_m = 2.0f, float influence_radius_m = 6.0f,
        float max_avoid_speed_mps = 3.0f, float prediction_horizon_s = 2.0f) const;

    [[nodiscard]] static std::vector<AvoidanceObstacle>
    obstacles_from_lidar(const sensors::LidarMeasurement& scan,
                         const Eigen::Vector3d& drone_position, size_t stride = 24,
