...
public:
    ...
    [[nodiscard]] VisualFeatureTrackBatch update(
        ...
        const std::size_t count = std::min(previous_pixels.size(), current_pixels.size());
...
private:
    ...
    triangulate_two_view(const Eigen::Vector2d& previous_pixel,
        ...
        Eigen::Vector3d ray_previous =
            previous_pose.R_wb() * K_inv * Eigen::Vector3d{previous_pixel.x(), previous_pixel.y(), 1.0};
        Eigen::Vector3d ray_current =
            current_pose.R_wb() * K_inv * Eigen::Vector3d{current_pixel.x(), current_pixel.y(), 1.0};
