...
struct Keyframe {
    ...
    [[nodiscard]] size_t serialized_size() const {
        return descriptors.total() * descriptors.elemSize() +
               keypoints.size() * sizeof(cv::KeyPoint) + sizeof(Keyframe);
    }
...
public:
    ...
    explicit KeyframeManager(uint32_t drone_id, std::shared_ptr<swarm::V2XMeshNetwork> net,
                             KeyframeSelectionPolicy policy = {});

    //  Add a new observation
    //   Returns new Keyframe ID if one was created, else std::nullopt
    std::optional<uint64_t> try_add_frame(const cv::Mat& image, const Eigen::Vector3d& pos,
                                          const Eigen::Quaterniond& ori, double timestamp);
    ...
    [[nodiscard]] std::vector<uint64_t> find_loop_closure_candidates(const cv::Mat& query_desc,
                                                                     size_t top_k = 5) const;
    [[nodiscard]] std::optional<RelocalizationResult>
    attempt_relocalization(const cv::Mat& image, const Eigen::Vector3d& pose_guess,
...
private:
    bool should_create_keyframe(const Eigen::Vector3d& pos, const Eigen::Quaterniond& ori,
                                double ts, size_t tracked_features) const;
    ...
    std::vector<uint64_t> extract_and_triangulate(const cv::Mat& image, Keyframe& kf);
