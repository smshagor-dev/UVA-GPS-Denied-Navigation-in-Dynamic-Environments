...
VisualFrontendResult run_visual_frontend(const cv::Mat& previous_gray, const cv::Mat& current_gray,
    ...
    for (int r = 0; r < 3; ++r) for (int c = 0; c < 3; ++c) K_cv.at<double>(r, c) = K(r, c);
    ...
    int inlier_count = 0;
    for (int i = 0; i < inlier_mask.rows; ++i) if (inlier_mask.at<uchar>(i, 0) != 0) ++inlier_count;
    result.metrics.inlier_ratio = tracked_curr.empty() ? 0.0 :
        static_cast<double>(inlier_count) / static_cast<double>(tracked_curr.size());

    cv::Mat R_cv, t_cv, recover_mask;
...
VisualFrontendResult build_placeholder_visual_frontend_result(const sensors::CameraFrame& frame,
    ...
    size_t accepted = 0;
    for (const auto& det : frame.detections) if (det.confidence >= 0.6f) ++accepted;
...
void VIOPipeline::attach_imu(std::shared_ptr<sensors::IMUSensor> imu) {
    imu_ = std::move(imu); imu_->set_data_callback([this](const sensors::ImuMeasurement& m) { enqueue(m); });
}
void VIOPipeline::attach_camera(std::shared_ptr<sensors::CameraSensor> cam) {
    cam_ = std::move(cam); cam_->set_data_callback([this](const sensors::CameraFrame& f) { enqueue(f); });
}
void VIOPipeline::attach_lidar(std::shared_ptr<sensors::LidarSensor> lidar) {
    lidar_ = std::move(lidar); lidar_->set_data_callback([this](const sensors::LidarMeasurement& m) { enqueue(m); });
}

bool VIOPipeline::start() {
    if (running_.exchange(true)) return true;
    if (coordinator_) { coordinator_->initialize(); (void)coordinator_->start(); }
...
void VIOPipeline::reset() {
    ...
    last_imu_ts_ = -1.0; last_camera_ts_ = -1.0; measurement_sequence_ = 0;
    previous_gray_frame_.release(); previous_camera_pose_valid_ = false;
    visual_feature_tracks_.reset();
    { std::lock_guard visual_lock(visual_metrics_mutex_); last_visual_metrics_ = {}; }
...
void VIOPipeline::enqueue(SensorEvent evt) {
    { std::lock_guard lock(queue_mutex_); if (event_queue_.size() > 2000) event_queue_.pop();
...
void VIOPipeline::processing_loop() {
    while (running_.load()) {
        ...
        { std::unique_lock lock(queue_mutex_);
          ...
          evt = std::move(event_queue_.front()); event_queue_.pop(); }
...
void VIOPipeline::submit_tracked_features_to_shadow(const VisualFrontendResult& frontend_result,
    ...
    if (!coordinator_ || frontend_result.previous_inlier_pixels.empty() ||
        frontend_result.previous_inlier_pixels.size() != frontend_result.current_inlier_pixels.size()) return;
    ...
    { std::lock_guard lock(runtime_mutex_); ++runtime_telemetry_.shadow_feature_batches_submitted; }
}
...
void VIOPipeline::handle(const sensors::CameraFrame& frame) {
    ...
    else frontend_result.metrics.visual_update_confidence = kDefaultVisualConfidenceOnFailure;
    ...
    if (frontend_result.metrics.update_accepted) {
        ...
        payload.sigma_position_m = std::clamp(0.50 - frontend_result.metrics.visual_update_confidence * 0.28, 0.14, 0.50);
        payload.sigma_velocity_mps = std::clamp(0.65 - frontend_result.metrics.visual_update_confidence * 0.30, 0.18, 0.65);
        ...
        (void)coordinator_->process_measurement(make_visual_pose_envelope(
            VisualPoseMeasurementPayload{predicted_pose.position, predicted_pose.velocity, 0.35, 0.45},
            MeasurementStamp{frame.timestamp, measurement_sequence_++}));
    }

    { std::lock_guard lock(visual_metrics_mutex_); last_visual_metrics_ = frontend_result.metrics; }
    { std::lock_guard lock(runtime_mutex_);
...
void VIOPipeline::handle(const sensors::LidarMeasurement& lidar) {
    if (!coordinator_ || !coordinator_->active_snapshot().initialized || !lidar.cloud || lidar.cloud->empty()) return;
    std::vector<float> z_vals; z_vals.reserve(lidar.cloud->size());
    for (const auto& pt : *lidar.cloud) if (pt.z > -20.0f && pt.z < 0.5f) z_vals.push_back(pt.z);
    if (z_vals.empty()) return;
    const auto median_index = z_vals.size() / 2;
    std::nth_element(z_vals.begin(), z_vals.begin() + static_cast<std::vector<float>::difference_type>(median_index), z_vals.end());
...
void VIOPipeline::apply_visual_quality_to_pose(PoseEstimate& pose) const {
    ...
    { std::lock_guard lock(visual_metrics_mutex_); metrics = last_visual_metrics_; }
    if (metrics.visual_update_confidence <= 0.0) return;
    pose.localization_confidence = std::clamp(pose.localization_confidence *
        std::clamp(metrics.visual_update_confidence, 0.18, 1.0), 0.0, 1.0);
    if (metrics.tracked_feature_count < kMinTrackedFeatures || metrics.inlier_ratio < kMinInlierRatio ||
        metrics.reprojection_error > kMaxReprojectionErrorPx) {
        pose.localization_degraded = true;
        pose.localization_source = metrics.used_placeholder ? "simulation-placeholder-vision" : "low-visual-quality";
    }
...
void VIOPipeline::set_estimator_validation_config(const EstimatorValidationConfig& cfg) {
    estimator_validation_cfg_ = cfg; reconfigure_coordinator();
}
void VIOPipeline::set_shadow_msckf_config(const MsckfConfig& cfg) { shadow_msckf_cfg_ = cfg; reconfigure_coordinator(); }
void VIOPipeline::reconfigure_coordinator() {
    ...
    coordinator_->stop(); coordinator_->initialize();
    visual_feature_tracks_.reset();
    if (running_.load()) (void)coordinator_->start();
}
double VIOPipeline::drift_m() const { return current_pose().drift_m; }
void VIOPipeline::refresh_runtime_telemetry_from_coordinator() {
    ...
    runtime_telemetry_.shadow_divergence_active = diagnostics.last_comparison.valid &&
        (diagnostics.last_comparison.position_delta_norm_m > estimator_validation_cfg_.shadow_position_divergence_m ||
         diagnostics.last_comparison.velocity_delta_norm_mps > estimator_validation_cfg_.shadow_velocity_divergence_mps ||
         diagnostics.last_comparison.orientation_delta_deg > estimator_validation_cfg_.shadow_orientation_divergence_deg);
