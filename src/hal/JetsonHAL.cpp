...
Platform detect_platform() {
    ...
    if (nv.is_open()) {
        std::string line;
        std::getline(nv, line);
        if (line.find("R36") != std::string::npos || line.find("R35") != std::string::npos)
    ...
    if (model.is_open()) {
        std::string m;
        std::getline(model, m);
        if (m.find("Raspberry Pi 5") != std::string::npos)
            return Platform::RPI5;
        if (m.find("Raspberry Pi 4") != std::string::npos)
            return Platform::RPI4;
    }
...
std::string_view to_string(Platform p) {
    ...
    case Platform::JETSON_NANO:
        return "Jetson Nano";
    case Platform::JETSON_ORIN:
        return "Jetson Orin";
    case Platform::RPI4:
        return "Raspberry Pi 4";
    case Platform::RPI5:
        return "Raspberry Pi 5";
    case Platform::X86_SIM:
        return "x86 Simulation";
    default:
        return "Unknown";
    }
}
...
SystemStats read_system_stats() {
    ...
    s.cpu_temp_c =
        read_sysfs_float("/sys/class/thermal/thermal_zone0/temp", 0.001f); // millideg  deg

    // Jetson GPU
    if (detect_platform() == Platform::JETSON_NANO || detect_platform() == Platform::JETSON_ORIN) {
        s.gpu_temp_c = read_sysfs_float("/sys/class/thermal/thermal_zone1/temp", 0.001f);
...
ESP32CamInterface::ESP32CamInterface() : ESP32CamInterface(Config{}) {}

ESP32CamInterface::ESP32CamInterface(Config cfg) : cfg_(std::move(cfg)) {
    logger_ = spdlog::get("HAL_ESP32");
    if (!logger_)
        logger_ = spdlog::stdout_color_mt("HAL_ESP32");
}
...
bool ESP32CamInterface::connect() {
    ...
    logger_->info("[ESP32CAM] Connected. {}Ã—{} @ {}fps", cfg_.width, cfg_.height, cfg_.fps_target);
    return true;
}
...
bool ESP32CamInterface::is_connected() const {
    return connected_;
}
...
bool ESP32CamInterface::grab_frame(cv::Mat& out, double& ts) {
    ...
    ts = std::chrono::duration<double>(std::chrono::steady_clock::now().time_since_epoch()).count();
...
bool ESP32CamInterface::try_reconnect() {
    for (int i = 0; i < cfg_.reconnect_attempts; ++i) {
        ...
        disconnect();
        std::this_thread::sleep_for(std::chrono::duration<double>(cfg_.reconnect_delay_s));
        if (connect())
            return true;
    }
...
I2CDevice::I2CDevice(std::string bus, uint8_t addr) : bus_(std::move(bus)), addr_(addr) {}
...
bool I2CDevice::open() {
    ...
    if (fd_ < 0) {
        logger_->error("I2C open failed: {}", bus_);
        return false;
    }
