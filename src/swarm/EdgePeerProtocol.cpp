...
std::optional<Eigen::Vector3d> extract_vec3(const std::string& content, const std::string& key) {
    const std::regex pattern("\"" + key +
                                 "\"\\s*:\\s*\\[\\s*(-?[0-9]+(?:\\.[0-9]+)?)\\s*,\\s*(-?[0-9]+(?:"
                                 "\\.[0-9]+)?)\\s*,\\s*(-?[0-9]+(?:\\.[0-9]+)?)\\s*\\]",
                             std::regex::icase);
    std::smatch match;
    if (std::regex_search(content, match, pattern) && match.size() >= 4) {
        try {
            return Eigen::Vector3d{std::stod(match[1].str()), std::stod(match[2].str()),
...
public:
    ...
    std::optional<std::string> read_string() {
        ...
        if (!ensure(static_cast<size_t>(*len)))
            return std::nullopt;
...
bool read_cbor_payload(CborReader& reader, EdgePeerPacket& packet, std::string& error) {
    ...
    case EdgePacketType::HEARTBEAT: {
        if (*payload_size != 8) {
            error = "heartbeat payload shape mismatch";
            return false;
        }
        HeartbeatPayload p;
        ...
        if (!peer_count || !stale_peer_count || !battery || !motor || !link || !fault || !health ||
            !autonomy)
            break;
    ...
    case EdgePacketType::POSE_STATE: {
        if (*payload_size != 3) {
            error = "pose_state payload shape mismatch";
            return false;
        }
        PoseStatePayload p;
    ...
    case EdgePacketType::EDGE_HEALTH: {
        if (*payload_size != 5) {
            error = "edge_health payload shape mismatch";
            return false;
        }
        EdgeHealthPayload p;
        const auto health = reader.read_string();
        ...
        if (!health || !autonomy || !consensus || !bandwidth || !disconnected)
            break;
        p.edge_health_status = *health;
        p.autonomy_state = *autonomy;
    ...
    case EdgePacketType::OBSTACLE_DIGEST: {
        if (*payload_size != 4) {
            error = "obstacle_digest payload shape mismatch";
            return false;
        }
        ...
        if (!local || !shared || !freshness || !digest)
            break;
    ...
    case EdgePacketType::THREAT_DIGEST: {
        if (*payload_size != 3) {
            error = "threat_digest payload shape mismatch";
            return false;
        }
    ...
    case EdgePacketType::CONSENSUS_STATE: {
        if (*payload_size != 5) {
            error = "consensus_state payload shape mismatch";
            return false;
        }
        ...
        if (!proposal || !state || !epoch || !quorum || !override)
            break;
    ...
    case EdgePacketType::EMERGENCY_CORRIDOR: {
        if (*payload_size != 4) {
            error = "emergency_corridor payload shape mismatch";
            return false;
        }
    ...
    case EdgePacketType::PEER_GOODBYE: {
        if (*payload_size != 1) {
            error = "peer_goodbye payload shape mismatch";
            return false;
        }
...
std::string_view to_string(EdgePacketType type) {
    ...
    case EdgePacketType::EDGE_HEALTH:
        return "edge_health";
    case EdgePacketType::OBSTACLE_DIGEST:
        return "obstacle_digest";
    case EdgePacketType::THREAT_DIGEST:
        return "threat_digest";
    case EdgePacketType::CONSENSUS_STATE:
...
std::optional<EdgePacketType> parse_edge_packet_type(std::string_view value) {
    ...
    if (normalized == "heartbeat")
        return EdgePacketType::HEARTBEAT;
    if (normalized == "pose_state")
        return EdgePacketType::POSE_STATE;
    if (normalized == "edge_health")
        return EdgePacketType::EDGE_HEALTH;
    if (normalized == "obstacle_digest")
        return EdgePacketType::OBSTACLE_DIGEST;
    if (normalized == "threat_digest")
        return EdgePacketType::THREAT_DIGEST;
    if (normalized == "consensus_state")
        return EdgePacketType::CONSENSUS_STATE;
    if (normalized == "emergency_corridor")
        return EdgePacketType::EMERGENCY_CORRIDOR;
    if (normalized == "peer_goodbye")
        return EdgePacketType::PEER_GOODBYE;
    return std::nullopt;
}

std::string_view to_string(ConsensusProposalType type) {
    switch (type) {
    case ConsensusProposalType::NONE:
        return "none";
    case ConsensusProposalType::COLLECTIVE_HALT:
        return "collective_halt";
    case ConsensusProposalType::EMERGENCY_REROUTE:
        return "emergency_reroute";
    case ConsensusProposalType::LEADER_CONTINUITY_HINT:
        return "leader_continuity_hint";
    case ConsensusProposalType::SPLIT_SWARM_RECOVERY_HINT:
        return "split_swarm_recovery_hint";
    }
    return "none";
}
...
std::optional<ConsensusProposalType> parse_consensus_proposal_type(std::string_view value) {
    const auto normalized = lowercase(value);
    if (normalized == "none")
        return ConsensusProposalType::NONE;
    if (normalized == "collective_halt")
        return ConsensusProposalType::COLLECTIVE_HALT;
    if (normalized == "emergency_reroute")
        return ConsensusProposalType::EMERGENCY_REROUTE;
    if (normalized == "leader_continuity_hint")
        return ConsensusProposalType::LEADER_CONTINUITY_HINT;
    if (normalized == "split_swarm_recovery_hint")
        return ConsensusProposalType::SPLIT_SWARM_RECOVERY_HINT;
    return std::nullopt;
}

std::string_view to_string(EdgeSerializationMode mode) {
    switch (mode) {
    case EdgeSerializationMode::JSON:
        return "json";
    case EdgeSerializationMode::CBOR:
        return "cbor";
    case EdgeSerializationMode::PROTOBUF_PLACEHOLDER:
...
std::optional<EdgeSerializationMode> parse_edge_serialization_mode(std::string_view value) {
    const auto normalized = lowercase(value);
    if (normalized == "json")
        return EdgeSerializationMode::JSON;
    if (normalized == "cbor")
        return EdgeSerializationMode::CBOR;
    if (normalized == "protobuf_placeholder")
        return EdgeSerializationMode::PROTOBUF_PLACEHOLDER;
    return std::nullopt;
}

std::string normalize_edge_source_tag(std::string_view value) {
    const auto normalized = lowercase(value);
    if (normalized == "real" || normalized == "simulation" || normalized == "playback" ||
...
std::string serialize_edge_packet_json(const EdgePeerPacket& packet) {
    ...
    case EdgePacketType::POSE_STATE:
        if (packet.pose_state.has_value()) {
            ...
            oss << "\"position_xyz\":[" << format_double(p.position.x()) << ","
                << format_double(p.position.y()) << "," << format_double(p.position.z()) << "],"
                << "\"velocity_xyz\":[" << format_double(p.velocity.x()) << ","
                << format_double(p.velocity.y()) << "," << format_double(p.velocity.z()) << "],"
    ...
    case EdgePacketType::EMERGENCY_CORRIDOR:
        if (packet.emergency_corridor.has_value()) {
            ...
            oss << "\"center_xyz\":[" << format_double(p.center.x()) << ","
                << format_double(p.center.y()) << "," << format_double(p.center.z()) << "],"
...
EdgePacketParseResult parse_edge_packet_json(std::string_view wire) {
    ...
    result.packet.source =
        normalize_edge_source_tag(extract_string(content, "source").value_or("unavailable"));
    ...
    case EdgePacketType::HEARTBEAT: {
        ...
        payload.stale_peer_count =
            static_cast<int>(extract_number(content, "stale_peer_count").value_or(0.0));
        ...
        payload.edge_health_status =
            extract_string(content, "edge_health_status").value_or("nominal");
        payload.autonomy_state =
            extract_string(content, "autonomy_state").value_or("distributed_autonomy");
    ...
    case EdgePacketType::POSE_STATE: {
        ...
        payload.localization_confidence =
            extract_number(content, "localization_confidence").value_or(0.0);
    ...
    case EdgePacketType::EDGE_HEALTH: {
        ...
        payload.consensus_state =
            extract_string(content, "consensus_state").value_or("single_node");
        payload.mesh_bandwidth_kbps = extract_number(content, "mesh_bandwidth_kbps").value_or(0.0);
        payload.disconnected_operation =
            extract_bool(content, "disconnected_operation").value_or(false);
        result.packet.edge_health = payload;
        break;
    }
    case EdgePacketType::OBSTACLE_DIGEST: {
        ObstacleDigestPayload payload;
        payload.local_obstacle_count =
            static_cast<int>(extract_number(content, "local_obstacle_count").value_or(0.0));
        payload.shared_obstacle_count =
            static_cast<int>(extract_number(content, "shared_obstacle_count").value_or(0.0));
        payload.freshness_ms =
            static_cast<uint32_t>(extract_number(content, "freshness_ms").value_or(0.0));
    ...
    case EdgePacketType::CONSENSUS_STATE: {
        ...
        payload.proposal_type =
            parse_consensus_proposal_type(extract_string(content, "proposal_type").value_or("none"))
                .value_or(ConsensusProposalType::NONE);
        payload.consensus_state =
            extract_string(content, "consensus_state").value_or("single_node");
        payload.consensus_epoch =
            static_cast<uint64_t>(extract_number(content, "consensus_epoch").value_or(0.0));
        payload.quorum_count =
            static_cast<int>(extract_number(content, "quorum_count").value_or(0.0));
        payload.local_safety_override =
            extract_bool(content, "local_safety_override").value_or(false);
    ...
    case EdgePacketType::EMERGENCY_CORRIDOR: {
        ...
        payload.hold_ttl_ms =
            static_cast<uint32_t>(extract_number(content, "hold_ttl_ms").value_or(0.0));
        payload.summary = extract_string(content, "summary").value_or("");
...
std::vector<uint8_t> serialize_edge_packet_cbor(const EdgePeerPacket& packet) {
    ...
    // [version, type, sender_id, timestamp_ms, sequence, trust_epoch, source, ttl_ms, auth_hook,
    // payload_array]
...
std::vector<uint8_t> serialize_edge_packet(const EdgePeerPacket& packet, EdgeSerializationMode mode,
    ...
    if (metrics != nullptr) {
        ...
        metrics->compression_ratio_vs_json =
            json_size == 0 ? 1.0
                           : static_cast<double>(wire.size()) / static_cast<double>(json_size);
        metrics->estimated = false;
    }
    return wire;
}

EdgePacketParseResult parse_edge_packet(std::span<const uint8_t> wire, EdgeSerializationMode mode,
                                        EdgeSerializationMetrics* metrics) {
    const auto started = std::chrono::steady_clock::now();
    ...
    if (mode == EdgeSerializationMode::CBOR) {
        ...
        parsed = parse_edge_packet_json(
            std::string_view(reinterpret_cast<const char*>(wire.data()), wire.size()));
    }
    const auto finished = std::chrono::steady_clock::now();
...
EdgePacketValidationResult validate_edge_packet(const EdgePeerPacket& packet,
    ...
    case EdgePacketType::HEARTBEAT:
        if (!packet.heartbeat.has_value())
            result.error = "missing heartbeat payload";
        break;
    case EdgePacketType::POSE_STATE:
        if (!packet.pose_state.has_value())
            result.error = "missing pose_state payload";
        break;
    case EdgePacketType::EDGE_HEALTH:
        if (!packet.edge_health.has_value())
            result.error = "missing edge_health payload";
        break;
    case EdgePacketType::OBSTACLE_DIGEST:
        if (!packet.obstacle_digest.has_value())
            result.error = "missing obstacle_digest payload";
        break;
    case EdgePacketType::THREAT_DIGEST:
        if (!packet.threat_digest.has_value())
            result.error = "missing threat_digest payload";
        break;
    case EdgePacketType::CONSENSUS_STATE:
        if (!packet.consensus_state.has_value())
            result.error = "missing consensus_state payload";
        break;
    case EdgePacketType::EMERGENCY_CORRIDOR:
        if (!packet.emergency_corridor.has_value())
            result.error = "missing emergency_corridor payload";
        break;
    case EdgePacketType::PEER_GOODBYE:
        if (!packet.peer_goodbye.has_value())
            result.error = "missing peer_goodbye payload";
        break;
    }
    ...
    if (packet.pose_state.has_value()) {
        if (!finite_vec3(packet.pose_state->position) ||
            !finite_vec3(packet.pose_state->velocity)) {
            result.error = "pose vectors must be finite";
            return result;
        }
    }
    if (packet.emergency_corridor.has_value()) {
        if (!finite_vec3(packet.emergency_corridor->center) ||
            packet.emergency_corridor->radius_m <= 0.0) {
