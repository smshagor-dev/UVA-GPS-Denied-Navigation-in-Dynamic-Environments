...
DecisionEngine::DecisionEngine(DecisionConfig cfg) : cfg_(cfg) {
    drone::utils::get_or_create_logger("AI")->info(
        "DecisionEngine initialized min_conf={} track_conf={} low_bat={} critical_bat={}",
        cfg_.min_detection_confidence, cfg_.target_track_confidence, cfg_.low_battery_pct,
...
DecisionCommand DecisionEngine::update(const DecisionContext& ctx) {
    ...
    logger->debug(
        "DecisionEngine::update battery={} inference_ready={} follower={} detections={} peers={}",
        ctx.system.battery_pct, ctx.inference_ready, ctx.swarm_follower,
        ctx.frame.has_value() ? ctx.frame->detections.size() : 0, ctx.swarm_peer_count);
    ...
    if (ctx.localization_lost || ctx.localization_confidence <= cfg_.lost_localization_threshold) {
        ...
        if (!ctx.camera_tracking_nominal || ctx.sync_confidence < 0.45 ||
            ctx.relocalization_count == 0) {
    ...
    if (ctx.localization_degraded ||
        ctx.localization_confidence <= cfg_.degraded_localization_threshold) {
    ...
    if (ctx.lidar_obstacle_count > 0 && ctx.nearest_lidar_obstacle_m >= 0.0 &&
        ctx.nearest_lidar_obstacle_m <= 2.2) {
        mode_ = BehaviorMode::AVOID_OBSTACLE;
        return build_lidar_avoid(ctx);
    }

    if (ctx.memory_prior.has_value() && ctx.memory_prior->recommend_caution && ctx.swarm_follower &&
    ...
    if (is_hazard_label(focus->detection.label) || is_unknown_label(focus->detection.label) ||
    ...
    return ctx.swarm_follower
               ? build_hold(
                     ctx,
                     "Detection seen but not mission-critical, follower yielding to swarm plan")
               : build_search(ctx);
}
...
std::optional<PerceptionFocus>
DecisionEngine::select_primary_detection(const sensors::CameraFrame& frame) const {
    auto logger = drone::utils::get_or_create_logger("AI");
    logger->debug("DecisionEngine::select_primary_detection detections={}",
    ...
    for (const auto& detection : frame.detections) {
        ...
        const float class_bias = is_hazard_label(detection.label)
                                     ? 1.25f
                                     : (is_unknown_label(detection.label)
                                            ? 1.15f
        ...
        candidate.score =
            detection.confidence * (0.45f + 0.35f * centered + 0.20f * area) * class_bias;
...
DecisionCommand DecisionEngine::build_emergency_land(const DecisionContext& ctx) const {
    drone::utils::get_or_create_logger("AI")->warn("DecisionEngine emergency land battery={}",
...
DecisionCommand DecisionEngine::build_return_home(const DecisionContext& ctx) const {
    drone::utils::get_or_create_logger("AI")->info(
        "DecisionEngine return-home battery={} tdoa_conf={:.2f}", ctx.system.battery_pct,
        ctx.tdoa_confidence);
    DecisionCommand command;
    ...
    to_home.z() =
        std::clamp(static_cast<double>(cfg_.nominal_altitude_m) - ctx.pose.position.z(), -0.8, 0.8);
...
DecisionCommand DecisionEngine::build_hold(const DecisionContext& ctx, std::string summary) const {
    drone::utils::get_or_create_logger("AI")->debug(
        "DecisionEngine hold velocity=[{:.2f},{:.2f},{:.2f}] reason={}", ctx.pose.velocity.x(),
        ctx.pose.velocity.y(), ctx.pose.velocity.z(), summary);
    DecisionCommand command;
...
DecisionCommand DecisionEngine::build_search(const DecisionContext& ctx) const {
    drone::utils::get_or_create_logger("AI")->debug(
        "DecisionEngine search miss_count={} caution={}", target_miss_count_, caution_scale(ctx));
    DecisionCommand command;
    ...
    const double altitude_error =
        static_cast<double>(cfg_.nominal_altitude_m) - ctx.pose.position.z();
    command.desired_velocity = body_forward(ctx.pose) * (cfg_.max_search_speed_mps * speed_scale) +
                               body_up(ctx.pose) * std::clamp(altitude_error * 0.18, -0.5, 0.5);

    command.desired_velocity =
        clamp_speed(command.desired_velocity, cfg_.max_search_speed_mps * speed_scale);
    command.desired_yaw_rate_rads = (target_miss_count_ % 2 == 0) ? 0.18 : -0.18;
    command.summary =
        ctx.memory_prior.has_value() && ctx.memory_prior->recommend_caution
...
DecisionCommand DecisionEngine::build_hover_and_scan(const DecisionContext& ctx) const {
    drone::utils::get_or_create_logger("AI")->warn(
        ...
        ctx.localization_confidence, ctx.sync_confidence, ctx.camera_tracking_nominal);
    DecisionCommand command;
    ...
    command.desired_velocity =
        ...
        body_up(ctx.pose) *
            std::clamp(static_cast<double>(cfg_.safe_altitude_m - 1.0f) - ctx.pose.position.z(),
                       -0.15, 0.25);
    command.desired_velocity =
        clamp_speed(command.desired_velocity, cfg_.max_recovery_speed_mps * 0.65);
...
DecisionCommand DecisionEngine::build_safe_return_by_anchor(const DecisionContext& ctx) const {
    drone::utils::get_or_create_logger("AI")->warn(
        "DecisionEngine safe-return anchor_count={} tdoa_conf={:.2f}", ctx.visible_anchor_count,
        ctx.tdoa_confidence);
    DecisionCommand command;
    ...
    recovery.z() =
        std::clamp(static_cast<double>(cfg_.safe_altitude_m) - ctx.pose.position.z(), -0.25, 0.45);
...
DecisionCommand DecisionEngine::build_localization_degraded(const DecisionContext& ctx) const {
    drone::utils::get_or_create_logger("AI")->warn(
        "DecisionEngine degraded localization conf={:.2f} source={}", ctx.localization_confidence,
    ...
    const double altitude_error =
        static_cast<double>(cfg_.nominal_altitude_m) - ctx.pose.position.z();
    command.desired_velocity = (-ctx.pose.velocity * 0.45) +
    ...
    command.summary =
        ctx.tdoa_confidence < 0.2
            ? "Localization degraded, cautious dead-reckoning while anchor aid is unavailable"
            : "Localization degraded, slowing down and stabilizing for recovery using " +
                  ctx.localization_source;
    return command;
}
...
DecisionCommand DecisionEngine::build_localization_lost(const DecisionContext& ctx) const {
    drone::utils::get_or_create_logger("AI")->error(
        ...
        ctx.localization_confidence, ctx.tdoa_confidence);
    DecisionCommand command;
    ...
    if (ctx.tdoa_position.has_value() && ctx.tdoa_confidence >= cfg_.tdoa_recovery_confidence) {
        Eigen::Vector3d recovery = *ctx.tdoa_position - ctx.pose.position;
        recovery.z() = std::clamp(static_cast<double>(cfg_.safe_altitude_m) - ctx.pose.position.z(),
                                  -0.3, 0.5);
        command.desired_velocity = clamp_speed(recovery, cfg_.max_recovery_speed_mps);
    ...
    command.desired_velocity =
        ...
        body_up(ctx.pose) *
            std::clamp(static_cast<double>(cfg_.safe_altitude_m - 1.5f) - ctx.pose.position.z(),
                       -0.2, 0.35);
    command.desired_velocity =
        clamp_speed(command.desired_velocity, cfg_.max_recovery_speed_mps * 0.8);
...
DecisionCommand DecisionEngine::build_track(const DecisionContext& ctx,
    ...
    drone::utils::get_or_create_logger("AI")->info(
        "DecisionEngine track label={} conf={:.3f} area={:.3f}", focus.detection.label,
        focus.detection.confidence, focus.normalized_area);
    DecisionCommand command;
    ...
    Eigen::Vector3d velocity =
        body_forward(ctx.pose) *
            std::clamp(area_error * 8.0f, -0.3f,
    ...
    command.desired_yaw_rate_rads =
        std::clamp(static_cast<double>(focus.image_offset.x()) * 1.8, -0.9, 0.9);

    std::ostringstream oss;
    oss << "Tracking " << focus.detection.label << " conf=" << focus.detection.confidence
...
DecisionCommand DecisionEngine::build_avoid(const DecisionContext& ctx,
                                            const PerceptionFocus& focus) const {
    drone::utils::get_or_create_logger("AI")->warn(
        "DecisionEngine avoid label={} score={:.3f} area={:.3f}", focus.detection.label,
        focus.score, focus.normalized_area);
    DecisionCommand command;
    ...
    Eigen::Vector3d velocity =
        ...
        -body_right(ctx.pose) *
            std::clamp(static_cast<double>(focus.image_offset.x()) * 3.0, -1.2, 1.2) +
    ...
    command.desired_velocity = clamp_speed(velocity, cfg_.max_avoid_speed_mps * caution_scale(ctx) *
                                                         localization_scale(ctx));
    command.desired_yaw_rate_rads =
        -std::clamp(static_cast<double>(focus.image_offset.x()) * 1.6, -1.0, 1.0);
    command.requires_operator_attention = is_hazard_label(focus.detection.label) ||
                                          is_unknown_label(focus.detection.label) ||
                                          focus.normalized_area >= 0.20f;

    std::ostringstream oss;
    oss << "Avoiding " << focus.detection.label << " score=" << focus.score;
    command.summary = oss.str();
    return command;
}
...
DecisionCommand DecisionEngine::build_lidar_avoid(const DecisionContext& ctx) const {
    drone::utils::get_or_create_logger("AI")->warn(
        "DecisionEngine lidar avoid obstacle_count={} nearest={:.2f}m", ctx.lidar_obstacle_count,
    ...
    Eigen::Vector3d velocity =
        ...
        body_up(ctx.pose) *
            std::clamp(static_cast<double>(cfg_.safe_altitude_m) - ctx.pose.position.z(), 0.0, 0.7);
    command.desired_velocity = clamp_speed(velocity, cfg_.max_avoid_speed_mps * caution_scale(ctx) *
                                                         localization_scale(ctx));
